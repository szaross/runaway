...
from runaway.floors import Cell2D, Cell3D, TransferLink
...
class EvacuationModel(Model):
    ...
    def __init__(self, config: SimulationConfig) -> None:
        ...
        spawn_points = [(spec.level, x, y) for spec in self.floor_specs for x, y in spec.spawn_points]
        ...
        for floor, walls in self.walls_by_floor.items():
            for x, y in walls:
                self.grid.place_agent(WallCell(f"wall-{floor}-{x}-{y}", self), self.to_grid_pos((floor, x, y)))

        for floor, exits in self.exits_by_floor.items():
            for x, y in exits:
                self.grid.place_agent(ExitCell(f"exit-{floor}-{x}-{y}", self), self.to_grid_pos((floor, x, y)))
