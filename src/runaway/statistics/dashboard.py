    ...
    def render(self, model) -> str:
        ...
        for label, value, bg in tiles:
            tiles_html += (
                ...
                f"justify-content:center;background:{bg};padding:16px 6px;"
                f'border-radius:8px">'
                f'<div style="font-size:26px;font-weight:700;color:#1e293b">'
                f"{value}</div>"
        ...
        if model.config.floors_count > 1:
            floors_html = '<div style="margin-top:10px;display:flex;flex-direction:column;gap:4px">'
            for floor in range(model.config.floors_count):
                # Count agents that started on this floor
                started_on_floor = sum(
                    1 for a in model._active_agents if a.start_floor == floor
                ) + sum(1 for r in records if r.get("start_floor") == floor)
                ...
                floors_html += (
                    ...
                    f'background:#f8fafc;padding:10px 10px;border-radius:6px;font-size:13px">'
